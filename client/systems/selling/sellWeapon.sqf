...
storeSellingHandle = [] spawn
{
	...
	// Calculating weapon sell value
	{
		...
		{
			_sellValue = GET_EIGHTH_PRICE(_x select 2);
		};
	...
	// Add ammo-based price of magazines to total sell value, and their names to confirm message
	{
		...
		{
			_sellValue = _sellValue + GET_EIGHTH_PRICE(_magValue * (_x / _magFullAmmo)); // Get selling price relative to ammo count
		} forEach _magAmmos;
		...
		if (_x != "") then
		{
...

			{
				if (_x select 1 == _wepItem) exitWith
				{
					...
					_itemValue = GET_EIGHTH_PRICE(_x select 2);
				};
